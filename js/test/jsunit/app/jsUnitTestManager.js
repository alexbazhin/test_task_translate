...
// seconds to wait for each test_task_translate page to load
JsUnitTestManager.TESTPAGE_WAIT_SEC = 10;

// milliseoncds between test_task_translate runs
...
JsUnitTestManager.prototype._handleNewSuite = function () {
    ...
    if (allegedSuite.isJsUnitTestSuite) {
        ...
        this.fatalError('Invalid test_task_translate suite in file ' + this._currentTestPage.url);
        this.abort();
    }
}
...
JsUnitTestManager.prototype._runTest = function () {
    ...
    if (this._testIndex == 0) {
        ...
        if (typeof(this.testFrame.setUpPage) == 'function') {
            // first test_task_translate for this page and a setUpPage is defined
            if (typeof(this.testFrame.setUpPageStatus) == 'undefined') {
                ...
                // try test_task_translate again later
    ...
    // either not first test_task_translate, or no setUpPage defined, or setUpPage completed
...
JsUnitTestManager.prototype.getTestFunctionNamesFromRuntimeObject = function (testFrame) {
    ...
    if (testFrame.RuntimeObject) {
        var runtimeObject = testFrame.RuntimeObject("test_task_translate*");
...
JsUnitTestManager.prototype._indexOfTestFunctionIn = function (string) {
    return string.indexOf('function test_task_translate');
}
...
JsUnitTestManager.prototype.executeTestFunction = function (theTest) {
    ...
    this.setStatus('Running test_task_translate "' + this._testFunctionName + '"');
    var exception = null;
    ...
    try {
        ...
        try {
            ...
            //Unlike JUnit, only assign a tearDown exception to excep if there is not already an exception from the test_task_translate body
            if (exception == null)
                exception = e2;
        }
    }
...
JsUnitTestManager.prototype.resolveUserEnteredTestFileName = function (rawText) {
    ...
    // only test_task_translate for file:// since Opera uses a different format
